...
1. Генерируем данные для основного модуля index.js:
  ...
  1.2 Массив условий и правильных ответов в цикле for:
    1.2.1 Массив условий: каждая запись содержит 2 случайных целых числа от 1 до 10 и
    1 случайное число от 1 до 3,
    ...
    1.2.2 Массив правильных ответов: записывается число, которое является результатом
    вычисления выражения, полученного в п. 1.2.1
...
export default () => {
  ...
  for (let i = 0; i < 9; i += 1) {
    ...
    // 2 - операция вычитания
    // 3 - операция умножения
    // 1.2.2 Массив правильных ответов: записывается число, которое является результатом вычисления
    // выражения, полученного в п. 1.2.1
    switch (operation) {
      case 1:
        currentQuestion += ' + ';
